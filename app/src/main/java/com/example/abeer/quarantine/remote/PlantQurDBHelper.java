...
//            }
    ...
    public int Insert_Dashforanything(String Result) {
        database = engine.getWritableDatabase();
        values = new ContentValues();
        values.put("TextJsoinSendServer", Result);
        int ID = (int) database.insert("DashForanything", null, values);
        database.close();
    ...
    public int update_counterResultForAdmin(Context context, String ipadrass, String CountColumnName, long checkRequestId, long Items_ID, long EmpId, boolean IsFinishedAll) {
        ...
        if (count == 0) {
            ...
            try {
                ...
                Insert_Dashforanything(totaldata.toString());
    ...
    public JSONArray Getdatatosendservercheckrequset(long RequestCommittee_ID, Long ID_Emp) throws JSONException {
        ...
        Cursor cursor = engine.getReadableDatabase().rawQuery("select I._id,I.0,I.ProdPlant_ID ,R.Committee_ID from RequestCommittee R INNER JOIN Items I ON R._id==I.RequestCommittee_ID where I.Has_Result=1 and I.RequestCommittee_ID=" + RequestCommittee_ID, null);
        Cursor items;
        int count_items;
        int count = cursor.getCount();
        if (count > 0) {
    ...
    public JSONArray Getdatatosendserverconfirmdata(String Tablename,Long Items_ID) throws JSONException {
        JSONArray Result = null;
        Cursor cursor = engine.getReadableDatabase().rawQuery("select JsonConfirmResult from "+Tablename+" where Items_ID=" + Items_ID, null);
        int count = cursor.getCount();
        if (count > 0) {
    ...
    public void FinalSEndForConfirm(Context context, String ipadrass, Long Items_ID) {

        JSONObject totaldata = new JSONObject();
        try {
            totaldata.put("Committe_Dto", Getdatatosendserverconfirmdata("CommitteeResult", Items_ID));
            totaldata.put("SampleDto", Getdatatosendserverconfirmdata("SampleData", Items_ID));
            totaldata.put("Treatment_Dto", Getdatatosendserverconfirmdata("TreatmentData", Items_ID));
            Insert_Dashforanything(totaldata.toString());
            Public_function public_function = new Public_function();
//            public_function.senddataonlinetoserverformoreprocess(totaldata, context, ipadrass + ApiCall.urConfirmResult);
